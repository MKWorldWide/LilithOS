...
# =============================================================================
# LilithOS Nintendo Switch SD Card Builder
# =============================================================================
# This script creates a complete LilithOS image for Nintendo Switch
# that can be injected via SD card and payload injection.
# =============================================================================
...
MOUNT_POINT="/mnt/lilithos_switch"
...
check_prerequisites() {
    ...
    if [[ $EUID -ne 0 ]]; then
        error "This script must be run as root for SD card operations"
        echo "Please run: sudo $0"
        exit 1
    fi
    
    # Check required tools
    local required_tools=("dd" "losetup" "mkfs.fat" "mount" "umount" "tar" "gzip")
...
create_switch_image() {
    ...
    dd if=/dev/zero of="${SWITCH_IMAGE_NAME}.img" bs=1M count=8192
    
    # Set up loop device
    local loop_device=$(losetup --find --show "${SWITCH_IMAGE_NAME}.img")
    log "🔗 Loop device: $loop_device"
    
    # Create partition table (MBR for Switch compatibility)
    parted "$loop_device" mklabel msdos
    parted "$loop_device" mkpart primary fat32 1MiB 100%
    
    # Get partition device
    local partition_device="${loop_device}p1"
    
    # Format partition as FAT32 (required for Switch)
    log "💾 Formatting partition as FAT32..."
    mkfs.fat -F 32 "$partition_device"
...
install_lilithos_core() {
    ...
    mkdir -p "$MOUNT_POINT/lilithos"
    mkdir -p "$MOUNT_POINT/lilithos/modules"
    mkdir -p "$MOUNT_POINT/lilithos/config"
    mkdir -p "$MOUNT_POINT/lilithos/bin"
    mkdir -p "$MOUNT_POINT/lilithos/lib"
    mkdir -p "$MOUNT_POINT/lilithos/share"
    
    # Copy core system files
    if [ -d "../../core" ]; then
        cp -r ../../core/* "$MOUNT_POINT/lilithos/"
    else
        create_minimal_core
    fi
    
    # Make executables
    chmod +x "$MOUNT_POINT/lilithos/bin"/*
...
install_switch_modules() {
    ...
    mkdir -p "$MOUNT_POINT/lilithos/modules/chips/nintendo-switch"
    mkdir -p "$MOUNT_POINT/lilithos/modules/features/joycon"
    mkdir -p "$MOUNT_POINT/lilithos/modules/features/switch_display"
    mkdir -p "$MOUNT_POINT/lilithos/modules/features/switch_power"
    
    # Copy Switch chip module
    if [ -d "../../../modules/chips/nintendo-switch" ]; then
        cp -r ../../../modules/chips/nintendo-switch/* "$MOUNT_POINT/lilithos/modules/chips/nintendo-switch/"
    fi
    
    # Copy Joy-Con module
    if [ -d "../../../modules/features/joycon" ]; then
        cp -r ../../../modules/features/joycon/* "$MOUNT_POINT/lilithos/modules/features/joycon/"
    fi
    ...
    find "$MOUNT_POINT/lilithos/modules" -name "*.sh" -exec chmod +x {} \;
...
install_switch_theme() {
    ...
    mkdir -p "$MOUNT_POINT/lilithos/modules/themes/dark-glass"
    
    # Create Switch-optimized theme
    cat > "$MOUNT_POINT/lilithos/modules/themes/dark-glass/theme.css" << 'EOF'
...
EOF
    ...
    cat > "$MOUNT_POINT/lilithos/modules/themes/dark-glass/init.sh" << 'EOF'
#!/bin/bash
# LilithOS Switch Dark Glass Theme
...
EOF
    ...
    chmod +x "$MOUNT_POINT/lilithos/modules/themes/dark-glass/init.sh"
...
create_switch_documentation() {
    ...
    cat > "$MOUNT_POINT/README_SWITCH.md" << 'EOF'
...
finalize_switch_image() {
    ...
    # Create version file
    cat > "$MOUNT_POINT/lilithos/VERSION" << EOF
LilithOS Nintendo Switch Edition
...
EOF
    ...
    cat > "$MOUNT_POINT/install_lilithos_switch.sh" << 'EOF'
#!/bin/bash
...
EOF
    ...
    chmod +x "$MOUNT_POINT/install_lilithos_switch.sh"
    
    success "Switch image finalized"
}

cleanup_and_compress() {
    log "🧹 Cleaning up and compressing..."
    ...
    # Create checksum
    sha256sum "${SWITCH_IMAGE_NAME}.img.gz" > "${SWITCH_IMAGE_NAME}.img.gz.sha256"
    
    success "Switch image compressed and ready"
}
...
main() {
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║                LilithOS Nintendo Switch Builder              ║"
    echo "║                    Tegra X1 Optimized                        ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    echo "2. Write the .img file to your SD card"
